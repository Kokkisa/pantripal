    ...
    <div>
      <div style={S.dh(aiResult ? "#059669" : "#1e1b18")}>
        <div style={{ display:"flex", alignItems:"center" }}>
          ...
          <div>
            <h1 style={{ fontSize:20, fontWeight:800, margin:0 }}>{aiResult ? "AI Filled ✨" : "I'll type it myself"}</h1>
            <p style={{ color: aiResult ? "#a7f3d0" : "#9ca3af", margin:0, fontSize:12 }}>Start typing and I'll suggest the right unit</p>
          </div>
        </div>
      </div>
      <div style={{ padding:"14px 14px 88px" }}>
        ...
        {aiError && <div style={{ background:"#fef3c7", borderRadius:12, padding:"10px 14px", marginBottom:10, border:"1px solid #fde68a" }}><div style={{ display:"flex", alignItems:"center", gap:8 }}><span style={S.alertBadge("#d97706")}>!</span><p style={{ margin:0, fontSize:12, fontWeight:600, color:"#92400e" }}>{aiError}</p></div></div>}
        <div style={S.card}>
          <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
            <div>
              <label style={S.label}>What is it? *</label>
              <input style={S.input} placeholder="e.g. Milk, Pampers Size 3, Olive Oil..." value={name} onChange={handleNameChange} />
              {name && unitSource && (
            ...
            <div>
              <label style={S.label}>Brand? (totally optional)</label>
              <input style={S.input} placeholder="e.g. Amul, Quaker, Pampers..." value={brand} onChange={e => setBrand(e.target.value)} />
            </div>
            <div>
              <label style={S.label}>What kind of thing is it?</label>
            ...
            <div>
              <label style={S.label}>How do you count it?</label>
