...
export default memo(function HomeTab({ userMeta, inventory, spaces, shoppingList, expiringItems, babyItems, dismissedHints, onDismissHint, onShowSearch, onShowShop, onShowProfile, onShowItemsList, onEditItem, onNavigateToSpace, onSetActiveTab, NavBar }) {
    ...
    <div>
      <div style={{ background:"#1e1b18", padding:"18px 18px 0", color:"white" }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
          <div>
            <p style={{ fontSize:11, color:"#9ca3af", margin:0, textTransform:"uppercase", letterSpacing:"0.10em" }}>{greeting}</p>
        ...
        {IS_DEMO && <div style={{ background:"rgba(217,119,6,0.15)", borderRadius:12, padding:"9px 13px", marginBottom:12, border:"1px solid rgba(217,119,6,0.3)" }}><p style={{ margin:0, fontSize:11, color:"#fcd34d", fontWeight:600 }}>🔧 You're in preview mode — nothing saves yet, but everything works.</p></div>}
        {(shoppingList.length>0||expiringItems.length>0) && (
          <div style={{ background:"rgba(217,119,6,0.15)", borderRadius:13, padding:"10px 13px", marginBottom:14, border:"1px solid rgba(217,119,6,0.3)" }}>
            {shoppingList.length>0&&<div style={{ display:"flex", alignItems:"center", gap:8 }}><span style={S.alertBadge("#d97706")}>!</span><p style={{ margin:0, fontSize:12, fontWeight:600, color:"#fcd34d" }}>{shoppingList.length} item{shoppingList.length>1?"s":""} need a restock run</p></div>}
            {expiringItems.length>0&&<p style={{ margin:"2px 0 0", fontSize:11, color:"#fbbf24" }}>⏰ {expiringItems.length} going off soon — use these first</p>}
          </div>
        )}
      </div>
      <div style={S.content}>
        ...
        {inventory.length === 0 ? (
          <div style={{ textAlign:"center", padding:"48px 20px" }}>
            ...
            <h2 style={{ fontSize:22, fontWeight:800, margin:"0 0 8px", color:"#1e1b18", letterSpacing:"-0.5px" }}>Your pantry is empty!</h2>
            <p style={{ fontSize:14, color:"#6b7280", margin:"0 0 24px", lineHeight:1.5 }}>Add your first item and we'll start keeping track of everything for you.</p>
            <button onClick={()=>onSetActiveTab("add")} style={S.btn("#D97706")}>+ Add your first item</button>
          </div>
        ) : (
          <>
            <div style={{ display:"flex", gap:8, marginBottom:14 }}>
              {[
                ...
                { label:"Running Low", value:shoppingList.length, icon:"🔴", color:"#fff7ed", accent:"#ea580c", action:onShowShop },
                { label:"Use Soon", value:expiringItems.length, icon:"⏰", color:"#fdf4ff", accent:"#9333ea", action:()=>onSetActiveTab("insights") },
              ].map((s,i)=>(
                <div key={i} onClick={s.action} style={{ flex:1, background:s.color, borderRadius:14, padding:"10px 6px", textAlign:"center", cursor:"pointer", boxShadow: "0 1px 4px rgba(0,0,0,0.06)", transition:"transform 0.1s" }}>
                  ...
                  <div style={{ fontSize:9, color:"#6b7280", fontWeight:700, textTransform:"uppercase", letterSpacing:"0.10em" }}>{s.label}</div>
                </div>
              ))}
            </div>
            ...
            {babyItems.length>0&&(
              <>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
                  ...
                  <span onClick={()=>onNavigateToSpace(spaces.find(s=>s.id==="s3"))} style={{ fontSize:12, color:"#9333ea", fontWeight:700, cursor:"pointer" }}>Check it →</span>
                </div>
                <div style={{ display:"flex", gap:8, marginBottom:14 }}>
                  {babyItems.map(item=>{
                    ...
                    return(
                      <div key={item.id} onClick={() => onEditItem(item)} style={{ flex:1, background:level!=="good"?SB[level]:"#fdf4ff", borderRadius:14, padding:"11px", border:`1.5px solid ${level!=="good"?SBo[level]:"#e9d5ff"}`, cursor:"pointer" }}>
                        ...
                        {level!=="good"&&<p style={{ margin:"2px 0 0", fontSize:9, color:SC[level], fontWeight:700 }}>Reorder!</p>}
                      </div>
                    );
                  })}
                </div>
            ...
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
              <h2 style={{ fontSize:14, fontWeight:800, margin:0 }}>Where things live</h2>
              <span onClick={()=>onSetActiveTab("spaces")} style={{ fontSize:12, color:"#d97706", fontWeight:700, cursor:"pointer" }}>All spaces →</span>
            </div>
            {spaces.map(space=>{
                ...
                <div key={space.id} onClick={()=>onNavigateToSpace(space)} style={{ background:space.color, borderRadius:16, padding:"12px 14px", marginBottom:8, cursor:"pointer", border:`1px solid ${space.accent}22` }}>
                  <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                    ...
                    <div style={{ flex:1 }}>
                      ...
                      <p style={{ margin:0, fontSize:11, color:"#6b7280" }}>{space.shelves.length} {space.shelves.length===1?"shelf":"shelves inside"} · {items.length} {items.length===1?"item":"items"} stored</p>
                    </div>
                    {low>0&&<span style={{ background:"#fff7ed", color:"#ea580c", fontSize:10, fontWeight:800, borderRadius:8, padding:"3px 8px" }}>{low} running low</span>}
            ...
            {expiringItems.length>0&&(
              <>
                <h2 style={{ fontSize:14, fontWeight:800, margin:"14px 0 8px" }}>Use these before they go ⏰</h2>
                <div style={S.card}>
                  {expiringItems.map((item,i)=>(
                    <div key={item.id} onClick={() => onEditItem(item)} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"9px 0", borderBottom:i<expiringItems.length-1?"1px solid #f5f5f4":"none", cursor:"pointer" }}>
                      ...
                      <span style={{ background:"#fef3c7", color:"#92400e", borderRadius:9, padding:"3px 9px", fontSize:11, fontWeight:700 }}>Use today →</span>
                    </div>
                  ))}
                </div>
              </>
