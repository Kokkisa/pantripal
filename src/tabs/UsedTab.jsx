...
export default function UsedTab({
            ...
            <button
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  setActionError("Some items couldn't be deducted: " + err.message);
                } finally {
                  setBusyAction(null);
                }
              }}
              ...
              {isCooking ? "Cooking..." : "🍳 Cook This — Deduct Ingredients"}
            </button>
          </div>
        </div>
      );
        ...
        <div style={{ flex:1, overflowY:"auto", padding:"14px 14px 100px" }}>
          ...
          {canMake.length === 0 && almostMake.length === 0 && (
            <div style={{ ...S.emptyWrap, padding:"60px 20px" }}>
              ...
              <p style={{ ...S.emptyTitle, fontWeight:800 }}>Pantry looks bare!</p>
              <p style={S.emptySub}>Add items to your pantry to see what you can cook</p>
            </div>
          )}
        </div>
      </div>
  ...
  if (usedScreen==="quick_use" && selConsume) {
    ...
    return (
      <div>
        <div style={S.dh()}><div style={{ display:"flex", alignItems:"center" }}><button style={S.back} onClick={()=>{setUsedScreen("list");setActionError(null);}}>‹</button><h1 style={{ fontSize:20, fontWeight:800, margin:0 }}>I Used This</h1></div></div>
        <div style={S.content}>
          <div style={S.card}>
        ...
        <div style={{ padding:"0 16px" }}>
          ...
          <button style={{ ...S.btn(isUsing ? "#e5e7eb" : "#1e1b18", isUsing ? "#9ca3af" : "white"), marginTop: actionError ? 8 : 0 }} disabled={isUsing} onClick={async()=>{
            ...
            try {
              ...
              setActionError("Failed to log usage: " + err.message);
            } finally {
              setBusyAction(null);
            }
          }}>{isUsing ? "Logging..." : "Confirm Usage"}</button>
        </div>
        <NavBar />
      </div>
    );
  ...
  if (usedScreen==="done" && justSaved) return (
    <div style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", minHeight:"80vh", padding:24, textAlign:"center" }}>
      ...
      {justSaved?.cost && <p style={{ color:"#d97706", margin:"0 0 8px", fontSize:14, fontWeight:700 }}>💰 Cost: ₹{justSaved.cost.toFixed(2)}</p>}
      ...
      <div style={S.content}>
        ...
        {filtered.length===0 ? (
          <div style={{ ...S.emptyWrap, padding:"40px 0" }}>
            ...
            <p style={{ ...S.emptyTitle, margin:0 }}>No items yet</p>
            <p style={{ fontSize:12, margin:0 }}>Add items via the + tab first</p>
          </div>
        ) : filtered.map(item=>(
